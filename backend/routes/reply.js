        ...
        if (err) {    
            ...
            if(groupIdx == undefined){
                conn.query("SELECT AUTO_INCREMENT as auto FROM information_schema.tables WHERE table_name = 'reply' AND table_schema = DATABASE();", (err, result) => {
                    conn.query('insert into reply(postIdx, groupIdx, groupNum, depth, content, memberIdx) values(?, ?, 0, 0, ?, ?);', [postIdx, result[0].auto, content, memberIdx], (err, result) => {
                        conn.release();
                        if(err){
                            callback(err, null);
                ...
                conn.query('select ifnull(MIN(groupNum),0) as cnt from reply where groupIdx = ? and groupNum > ? and depth >= ?;', [result1[0].groupIdx, result1[0].groupNum, result1[0].depth], (err, result2) => {
                        ...
                        if(result2[0].cnt == 0){
                            conn.query('select ifnull(max(groupNum),0)+1 as cnt from reply where groupIdx = ?', [result1[0].groupIdx], (err, result3) => {
                                ...
                                conn.query('insert into reply(postIdx, groupIdx, groupNum, depth, content, memberIdx) values(?, ?, ?, ?, ?, ?)', [postIdx, groupIdx, result3[0].cnt, result1[0].depth+1, content, memberIdx], (err, result) => {
                                    conn.release();
                                    if(err){
                                        callback(err, null);
                                        console.log('select문 오류')
                            ...
                            conn.query('update reply set groupNum = groupNum +1 where groupIdx = ? and groupNum >= ?', [result1[0].groupIdx, result2[0].cnt], (err, result3) => {
                                ...
                                conn.query('insert into reply(postIdx, groupIdx, groupNum, depth, content, memberIdx) values(?, ?, ?, ?, ?, ?)', [postIdx, result1[0].groupIdx, result2[0].cnt, result1[0].depth+1, content, memberIdx]);
                                conn.release();
                                    if(err){
                                        callback(err, null);
                                        console.log('select문 오류')
